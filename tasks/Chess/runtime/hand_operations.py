...
class ChessHandOperationsMixin:
    ...
    def _discover_named_hand_cards(
        ...
        roi_x, roi_y, roi_width, roi_height = self.O_BADGE_AREA.roi
        cards = []
        for result in results:
            ...
            points = result.box
            left = max(0, min(int(point[0]) for point in points))
            right = min(roi_width, max(int(point[0]) for point in points))
            top = max(0, min(int(point[1]) for point in points))
            bottom = min(roi_height, max(int(point[1]) for point in points))
            if right <= left or bottom <= top:
                continue
            # 保留文字框中央一半作为卡牌点击区，随机点不会跨到相邻手牌。
            inset_x = (right - left) // 4
            inset_y = (bottom - top) // 4
            cards.append({
                'kind': expected_text,
                'text': text,
                ...
                'click_roi': (
                    roi_x + left + inset_x,
                    roi_y + top + inset_y,
                    right - left - 2 * inset_x,
                    bottom - top - 2 * inset_y,
                ),
            })
    ...
    def discover_souls_from_hand(self) -> int:
        ...
        for _ in range(self.DISCOVER_SOUL_SAFETY_LIMIT):
            ...
            self.click(RuleClick(
                roi_front=card['click_roi'],
                roi_back=card['click_roi'],
                name='CHESS_DISCOVER_CARD',
            ))
