...
class ManualClaimMixin(PortraitUIMixin):
    ...
    def _run_manual_claim(self):
        ...
        while 1:
            ...
            # 每轮检查，避免登录、动画或无匹配分支的 continue 绕过总超时。
            if timeout.reached():
                break
            ...
            if self.appear(A.I_CLAIM_S):
                ...
                if matches:
                    ...
                    if target is not None:
                        _, x, y, w, h = target
                        # 保留本地随机点击方案，不修改共享模板的匹配区域。
                        click_target = copy(A.I_CLAIM)
                        click_target.roi_front = [x, y, w, h]
                        click_x, click_y = click_target.coord()
                        self._tap(click_x, click_y, name='I_CLAIM')
                        clicked_ys.append(y)
                        claimed_any = True
