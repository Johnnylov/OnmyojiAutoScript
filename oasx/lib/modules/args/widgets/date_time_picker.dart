...
class DateTimePickerBase extends StatefulWidget {
  DateTimePickerBase({
    ...
    this.hoverStyle,
    this.notHoverStyle,
    this.readableField = false,
  });

  String value = '';
  final TextStyle? hoverStyle;
  final TextStyle? notHoverStyle;
  final bool readableField;
...
class DateTimePickerBaseState extends State<DateTimePickerBase> {
  ...
  Widget build(BuildContext context) {
    if (widget.readableField) {
      final theme = Theme.of(context);
      final border = OutlineInputBorder(
        borderRadius: BorderRadius.circular(8),
        borderSide: BorderSide(
          color: theme.colorScheme.onSurfaceVariant.withValues(alpha: .38),
        ),
      );
      return InkWell(
        onTap: () => showPicker(context, widget.value),
        borderRadius: BorderRadius.circular(8),
